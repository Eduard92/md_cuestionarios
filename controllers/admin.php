...
class Admin extends Admin_Controller {
	...
	public function __construct()
	{
		...
		$this->rules = array(
            ...
            array(
				'field' => 'tabla',
				'label' => 'Organización de las preguntas',
				'rules' => 'required'
				)
		);
	...
	function create(){
		...
		if($this->form_validation->run()){
			...
			if($id=$this->cuestionario_m->insert($data))
			{
				...
				$Cuestionario = $this->cuestionario_m->get_cuestionario($id);
				if($Cuestionario->tabla = 1)
				{
					
					$table = $Cuestionario->tabla;
					Cuestionario::SavePreguntas($id,$this->input->post('preguntas'),$table,$this->input->post('respuestas') );  
				}
               else
               	{
